...
from enums import OrderStatus, active_status_list, SheetsPage, UserRole
...
async def add_order_in_google_table(sh: Spreadsheet, row_num: int, row: list, order_id: int):
    ...
    sh.worksheet(SheetsPage.BASE.value).update(cell, [row[:26]])
    await asyncio.sleep (1)
    cell = f'ab{row_num}'
    # sh.sheet1.update (cell, row [27])
    sh.worksheet(SheetsPage.BASE.value).update (cell, row [27])
...
async def update_order_in_google_table(sh: Spreadsheet, row: list):
    ...
    find_row = sh.worksheet(SheetsPage.BASE.value).find (query=row[0], in_column=1)
...
def clear_new_order_table(sh: Spreadsheet, last_row: int):
    ...
    sh.worksheet(SheetsPage.EDIT.value).update(cell, del_list)
    sh.worksheet(SheetsPage.EDIT.value).format (cell, {"backgroundColor": col})
